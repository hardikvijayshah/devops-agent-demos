...
echo "============================================"
echo "Lab 4: Post-Deploy Regression - Injecting"
echo "============================================"
...
@app.route('/')
def index():
    # Performance regression: 5 second artificial delay
    time.sleep(5)
    return jsonify({
        'service': 'CI/CD Workshop',
        'version': '2.0.0',
        'hostname': socket.gethostname()
    })
...
def status():
    # Performance regression: 3 second delay
    time.sleep(3)
    return jsonify({
        'status': 'operational',
        'uptime': time.time() - start_time
...
def process():
    # Performance regression: 4 second delay
    time.sleep(4)
...
echo ""
echo "The application now has artificial delays:"
echo "  - / endpoint: 5 second delay"
echo "  - /api/status: 3 second delay"
echo "  - /api/process: 4 second delay"
echo "  - /health: no delay (so deployment succeeds)"
echo ""
echo "Pipeline will succeed fully, but post-deployment traffic"
echo "will trigger the ALB latency alarm."
echo ""
echo "Next steps:"
echo "  1. Wait for pipeline to complete successfully (~8 min)"
echo "  2. Generate traffic: ../../scripts/generate-traffic.sh ${STACK_NAME} 30 1"
echo "  3. Wait for latency alarm to fire"
echo "  4. Observe DevOps Agent investigation"
echo ""
